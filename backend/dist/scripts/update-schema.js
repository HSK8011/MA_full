...
const updateSchema = async () => {
    try {
        ...
        console.log('📊 Current MongoDB connection string:', config_1.config.mongoUri.substring(0, 35) + '...');
        ...
        // User model indexes
        const userIndexes = await mongoose_1.default.connection.db.collection('users').indexes();
        ...
        if (sampleUser) {
            const missingFields = [];
            if (!('phone' in sampleUser))
                missingFields.push('phone');
            if (!('timezone' in sampleUser))
                missingFields.push('timezone');
            if (!('isTwoFactorEnabled' in sampleUser))
                missingFields.push('isTwoFactorEnabled');
            if (!('isActivityLoggingEnabled' in sampleUser))
                missingFields.push('isActivityLoggingEnabled');
            if (!('lastPasswordChange' in sampleUser))
                missingFields.push('lastPasswordChange');
            if (missingFields.length > 0) {
                ...
                const updateResult = await userCollection.updateMany({ $or: missingFields.map(field => ({ [field]: { $exists: false } })) }, {
                    $set: {
                        phone: null,
        ...
        await safeCreateIndex('notificationpreferences', { userId: 1, type: 1 }, { unique: true, background: true });
        // QueueSetting indexes
        await safeCreateIndex('queuesettings', { userId: 1, accountId: 1 }, { unique: true, background: true });
        // Integration indexes
        await safeCreateIndex('integrations', { userId: 1, serviceId: 1 }, { unique: true, background: true });
        // ActivityLog indexes
        await safeCreateIndex('activitylogs', { userId: 1 }, { background: true });
        // Handle the special case for timestamp TTL index
        try {
            const activityLogsCollection = mongoose_1.default.connection.db.collection('activitylogs');
            ...
            if (collExists) {
                ...
                const existingTimestampIndex = indexes.find(idx => idx.name === 'timestamp_1' &&
                    idx.key &&
                    idx.key.timestamp === 1);
